...
void normalize_3d(std::vector<std::vector<int> > & joints3d, std::vector<float> & result) {
    ...
    for (int i=0; i < joints3d.size(); ++i) {
        ...
        result.push_back(5.2*static_cast<double>(joints3d[i][0]) / 1400.0);
        result.push_back(-4* static_cast<double>(joints3d[i][1]) / 1600.0);
        result.push_back(-5* static_cast<double>(joints3d[i][2]) / 1400.0);
