...
namespace mFitting {
    ...
    const double m_fitting_w2 = 6.633249581; // sqrt(44)
    const double m_fitting_w3 = 0.264575131; // sqrt(0.07)
    const double m_fitting_w4 = 0.331662479; // sqrt(0.11)
    ...
    struct ESMOOTHError {
        ...
        template<typename T> bool operator() (const T * const theta, const T * const d, T * residuals) const {
            ...
            residuals[0] = T(m_fitting_w3)*(T(global_3d[num * 3]) - T(pl_x[0]) - d[0]);
            residuals[1] = T(m_fitting_w3)*(T(global_3d[num * 3 + 1]) - T(pl_y[0]) - d[1]);
            residuals[2] = T(m_fitting_w3)*(T(global_3d[num * 3 + 2]) - T(pl_z[0]) - d[2]);
        ...
        //static ceres::CostFunction * Create(const double pl_x, const double pl_y, const double pl_z, int num) {
        static ceres::CostFunction * Create(std::vector<double> pl_x, std::vector<double> pl_y, std::vector<double> pl_z, int num) {
    ...
    struct EDEPTHError {
        ...
        template<typename T> bool operator() (const T * const theta, const T * const d, T * residuals) const {
            ...
            residuals[0] = T(m_fitting_w4)*(T(global_3d[num * 3 + 2]) - T(pl_z) - d[2]);
            return true;
        }
